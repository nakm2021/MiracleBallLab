...
const BUILDER_DEVICES_STORAGE_KEY = "miracle-ball-lab-builder-devices-v1";
const MIRACLE_DJ_STORAGE_KEY = "miracle-ball-lab-dj-v1";
const PET_STORAGE_KEY = "miracle-ball-lab-pet-v1";
...
type BuilderDeviceKind = "bumper" | "gravity" | "warp" | "fan" | "rareAmp";
type BuilderDevice = { id: string; kind: BuilderDeviceKind; xRatio: number; yRatio: number; angle?: number; pairId?: string; uses: number; createdAt: number };
type BuilderMode = BuilderDeviceKind | "erase" | null;
type MiracleDjState = { enabled: boolean; style: "soft" | "dance" | "cosmic"; volume: number; combo: number; lastBeatAt: number };
type MiraclePetState = { name: string; species: string; level: number; exp: number; mood: number; element: string; form: string; lastFedAt: number; createdAt: number; favoriteRank: string; totalFeed: number };

const BUILDER_DEVICE_LABELS: Record<BuilderDeviceKind, string> = {
    bumper: "反射板",
    gravity: "重力井戸",
    warp: "ワープ門",
    fan: "上昇ファン",
...
const BUILDER_DEVICE_COLORS: Record<BuilderDeviceKind, string> = {
    bumper: "#f97316",
    gravity: "#7c3aed",
    warp: "#06b6d4",
    fan: "#22c55e",
...
function createDefaultDjState(): MiracleDjState {
    return { enabled: true, style: "soft", volume: 0.36, combo: 0, lastBeatAt: 0 };
}

function createDefaultPetState(): MiraclePetState {
    return { name: "ミラ", species: "確率スライム", level: 1, exp: 0, mood: 70, element: "無属性", form: "ぷるぷる体", lastFedAt: 0, createdAt: Date.now(), favoriteRank: "SR", totalFeed: 0 };
}

function loadBuilderDevices(): BuilderDevice[] {
    const rows = safeJsonParse<BuilderDevice[]>(localStorage.getItem(BUILDER_DEVICES_STORAGE_KEY), []);
    return rows.filter((x) => x && typeof x.xRatio === "number" && typeof x.yRatio === "number" && typeof x.kind === "string").slice(0, 60);
}

function saveBuilderDevices(): void { localStorage.setItem(BUILDER_DEVICES_STORAGE_KEY, JSON.stringify(builderDevices)); }
function loadMiracleDjState(): MiracleDjState { return { ...createDefaultDjState(), ...safeJsonParse<Partial<MiracleDjState>>(localStorage.getItem(MIRACLE_DJ_STORAGE_KEY), {}) }; }
function saveMiracleDjState(): void { localStorage.setItem(MIRACLE_DJ_STORAGE_KEY, JSON.stringify(miracleDj)); }
function loadMiraclePetState(): MiraclePetState { return { ...createDefaultPetState(), ...safeJsonParse<Partial<MiraclePetState>>(localStorage.getItem(PET_STORAGE_KEY), {}) }; }
function saveMiraclePetState(): void { localStorage.setItem(PET_STORAGE_KEY, JSON.stringify(miraclePet)); }
...
let builderDevices: BuilderDevice[] = loadBuilderDevices();
let builderMode: BuilderMode = null;
let warpEntranceId: string | null = null;
let miracleDj: MiracleDjState = loadMiracleDjState();
let miraclePet: MiraclePetState = loadMiraclePetState();
let djAudioContext: AudioContext | null = null;
let petPanel: HTMLDivElement;
...
canvas.addEventListener("pointerdown", (event) => handleBoardPointerDown(event));
...
utilityButtons.appendChild(setTooltip(setButtonLabel(createButton("装置ビルダー", () => showDeviceBuilderPopup()), "装置ビルダー", "Builder"), "盤面に反射板・重力井戸・ワープ門などを配置します。", "Place devices on the board."));
utilityButtons.appendChild(setTooltip(setButtonLabel(createButton("奇跡DJ", () => showMiracleDjPopup()), "奇跡DJ", "DJ"), "実験結果に合わせて音が進化するライブ機能です。", "Live sound changes with experiment results."));
utilityButtons.appendChild(setTooltip(setButtonLabel(createButton("使い魔", () => showMiraclePetPopup()), "使い魔", "Pet"), "奇跡を食べて育つ研究所の相棒を表示します。", "View the lab pet that grows from miracles."));
...
function resetExperiment(startNow = false): void {
    ...
    Composite.add(engine.world, [...createWallsAndFloor(), ...createPins(), ...createDividers(), ...createPachinkoYakumonoSensors(), ...createBuilderDeviceBodies()]);
    applyWorldModeBodyStyles();
...
// ======================================================
// 180-degree features: board builder / Miracle DJ / pet
// ======================================================

function getBuilderDeviceWorldPosition(device: BuilderDevice): { x: number; y: number } {
    return { x: clamp(device.xRatio, 0.03, 0.97) * geometry.width, y: clamp(device.yRatio, 0.06, 0.90) * geometry.height };
}

function createBuilderDeviceBodies(): Matter.Body[] {
    const bodies: Matter.Body[] = [];
    for (const device of builderDevices) {
        const pos = getBuilderDeviceWorldPosition(device);
        const color = BUILDER_DEVICE_COLORS[device.kind] ?? "#94a3b8";
        let body: Matter.Body;
        if (device.kind === "bumper") {
            const w = clamp(86 * geometry.scale, 44, 130);
            const h = clamp(13 * geometry.scale, 7, 22);
            body = Bodies.rectangle(pos.x, pos.y, w, h, {
                isStatic: true,
                angle: device.angle ?? -0.28,
                render: { fillStyle: color, strokeStyle: "#fff7ed", lineWidth: Math.max(2, 2.5 * geometry.scale) } as any,
            });
        } else if (device.kind === "fan") {
            body = Bodies.rectangle(pos.x, pos.y, clamp(80 * geometry.scale, 42, 128), clamp(16 * geometry.scale, 8, 28), {
                isStatic: true,
                isSensor: true,
                render: { fillStyle: "rgba(34,197,94,.22)", strokeStyle: color, lineWidth: Math.max(2, 2 * geometry.scale) } as any,
            });
        } else {
            body = Bodies.circle(pos.x, pos.y, clamp(24 * geometry.scale, 14, 38), {
                isStatic: true,
                isSensor: device.kind !== "rareAmp",
                render: { fillStyle: device.kind === "rareAmp" ? color : "rgba(255,255,255,.12)", strokeStyle: color, lineWidth: Math.max(2, 2.4 * geometry.scale) } as any,
            });
        }
        (body as any).plugin = { isBuilderDevice: true, builderDeviceId: device.id, builderDeviceKind: device.kind };
        bodies.push(body);
    }
    return bodies;
}

function rebuildBuilderDevicesOnBoard(): void {
    const targets = engine.world.bodies.filter((body) => (body as any).plugin?.isBuilderDevice);
    for (const body of targets) Composite.remove(engine.world, body);
    Composite.add(engine.world, createBuilderDeviceBodies());
}

function getNearestBuilderDevice(x: number, y: number): BuilderDevice | null {
    let best: BuilderDevice | null = null;
    ...
    return best && bestDistance < Math.max(42 * geometry.scale, 36) ? best : null;
}

function placeBuilderDeviceAt(x: number, y: number): void {
    if (!builderMode) return;
    if (builderMode === "erase") {
        const target = getNearestBuilderDevice(x, y);
        if (!target) {
            showSoftToast("近くに削除できる装置がありません");
            return;
        }
        builderDevices = builderDevices.filter((x) => x.id !== target.id && x.pairId !== target.id);
        saveBuilderDevices();
        rebuildBuilderDevicesOnBoard();
        showSoftToast(`${BUILDER_DEVICE_LABELS[target.kind]}を撤去しました`);
        return;
    }

    if (builderDevices.length >= 36) {
        showSoftToast("装置は最大36個までです");
        return;
    }

    const id = `device-${Date.now()}-${Math.floor(appRandom() * 100000)}`;
    const base: BuilderDevice = {
        id,
        kind: builderMode,
        xRatio: clamp(x / geometry.width, 0.04, 0.96),
        yRatio: clamp(y / geometry.height, 0.08, 0.86),
        angle: appRandom() < 0.5 ? -0.28 : 0.28,
        uses: 0,
        createdAt: Date.now(),
    };

    if (builderMode === "warp" && warpEntranceId) {
        base.pairId = warpEntranceId;
        const entrance = builderDevices.find((x) => x.id === warpEntranceId);
        if (entrance) entrance.pairId = id;
        warpEntranceId = null;
        showSoftToast("ワープ門の出口を接続しました");
    } else if (builderMode === "warp") {
        warpEntranceId = id;
        showSoftToast("ワープ門の入口を置きました。次に出口を置いてください。");
    } else {
        showSoftToast(`${BUILDER_DEVICE_LABELS[builderMode]}を配置しました`);
    }

    builderDevices.push(base);
    saveBuilderDevices();
    rebuildBuilderDevicesOnBoard();
}
...
function handleBoardPointerDown(event: PointerEvent): void {
    if (builderMode) {
        ...
        placeBuilderDeviceAt(point.x, point.y);
        return;
    }
    activateNearestPin(event);
}

function applyBuilderDeviceEffects(drop: Matter.Body): void {
    const dropPlugin = (drop as any).plugin;
    if (!dropPlugin?.isDrop) return;
    ...
    for (const device of builderDevices) {
        ...
        const hitRadius = Math.max(30 * geometry.scale, 24);

        if (device.kind === "gravity" && distance < hitRadius * 4.2) {
            const power = (1 - distance / (hitRadius * 4.2)) * 0.0000065;
            Body.applyForce(drop, drop.position, { x: dx * power, y: dy * power });
        } else if (device.kind === "fan" && Math.abs(dx) < hitRadius * 2.8 && Math.abs(dy) < hitRadius * 1.7) {
            Body.applyForce(drop, drop.position, { x: (appRandom() - 0.5) * 0.00003, y: -0.00095 * geometry.scale });
            if (appRandom() < 0.012) addFloatingText("上昇ファン", drop.position.x, drop.position.y - 12 * geometry.scale, BUILDER_DEVICE_COLORS.fan);
        } else if (device.kind === "warp" && device.pairId && distance < hitRadius && dropPlugin.lastWarpDeviceId !== device.id) {
            const target = builderDevices.find((x) => x.id === device.pairId);
            if (target) {
                const to = getBuilderDeviceWorldPosition(target);
                Body.setPosition(drop, { x: to.x + (appRandom() - 0.5) * 16 * geometry.scale, y: to.y - 16 * geometry.scale });
                Body.setVelocity(drop, { x: drop.velocity.x * 0.7 + (appRandom() - 0.5) * 4 * geometry.scale, y: Math.min(drop.velocity.y, -3.5 * geometry.scale) });
                dropPlugin.lastWarpDeviceId = target.id;
                window.setTimeout(() => { if (dropPlugin.lastWarpDeviceId === target.id) dropPlugin.lastWarpDeviceId = ""; }, 700);
                addFloatingText("ワープ", to.x, to.y - 18 * geometry.scale, BUILDER_DEVICE_COLORS.warp);
                playMiracleDjPulse(520, 0.12, 0.09);
            }
        } else if (device.kind === "rareAmp" && distance < hitRadius * 1.8 && !dropPlugin.builderRareAmpTouched) {
            dropPlugin.builderRareAmpTouched = true;
            addScore(777, "DEVICE 奇跡増幅器", drop.position.x, drop.position.y - 20 * geometry.scale);
            if ((dropPlugin.kind ?? "normal") === "normal" && appRandom() < 0.025 * getProbabilityScale()) {
                dropPlugin.kind = "gold";
                dropPlugin.symbol = "金";
                drop.render.fillStyle = "#ffd700";
                drop.render.strokeStyle = "#fff4a8";
                drop.render.lineWidth = 3 * geometry.scale;
                addFloatingText("増幅変質", drop.position.x, drop.position.y - 18 * geometry.scale, BUILDER_DEVICE_COLORS.rareAmp);
                playMiracleDjForRank("SR");
            }
        }
    }
}

function drawBuilderDevices(context: CanvasRenderingContext2D): void {
    if (builderDevices.length === 0) return;
    context.save();
    for (const device of builderDevices) {
        const pos = getBuilderDeviceWorldPosition(device);
        const color = BUILDER_DEVICE_COLORS[device.kind] ?? "#94a3b8";
        ...
        if (device.kind === "bumper") {
            context.rotate(device.angle ?? -0.28);
            context.fillStyle = color;
            roundRect(context, -42 * geometry.scale, -9 * geometry.scale, 84 * geometry.scale, 18 * geometry.scale, 9 * geometry.scale);
            context.fill();
            context.strokeStyle = "#ffffff";
            context.lineWidth = Math.max(2, 2 * geometry.scale);
            context.stroke();
            context.fillStyle = "#fff";
            context.font = `900 ${Math.round(clamp(13 * geometry.scale, 10, 18))}px "Noto Sans JP", sans-serif`;
            context.fillText("反射", 0, 1);
        } else {
            context.beginPath();
            context.arc(0, 0, clamp(24 * geometry.scale, 14, 38), 0, Math.PI * 2);
            context.fillStyle = builderMode === device.kind ? `${color}55` : "rgba(255,255,255,.20)";
            context.strokeStyle = color;
            context.lineWidth = Math.max(2, 2.5 * geometry.scale);
            context.fill();
            context.stroke();
            context.font = `900 ${Math.round(clamp(18 * geometry.scale, 12, 28))}px "Noto Sans JP", "Segoe UI Emoji", sans-serif`;
            context.fillStyle = color;
            const icon = device.kind === "gravity" ? "重" : device.kind === "warp" ? "門" : device.kind === "fan" ? "風" : "増";
    ...
    if (builderMode) {
        ...
        roundRect(context, 12 * geometry.scale, 10 * geometry.scale, 280 * geometry.scale, 34 * geometry.scale, 12 * geometry.scale);
        context.fill();
        context.fillStyle = "#fff";
        context.font = `900 ${Math.round(clamp(15 * geometry.scale, 12, 22))}px "Noto Sans JP", sans-serif`;
        const label = builderMode === "erase" ? "撤去モード" : `${BUILDER_DEVICE_LABELS[builderMode]} 配置モード`;
        context.fillText(label, 152 * geometry.scale, 28 * geometry.scale);
    }
    context.restore();
}

function showDeviceBuilderPopup(): void {
    const rows = (Object.keys(BUILDER_DEVICE_LABELS) as BuilderDeviceKind[]).map((kind) => {
        const active = builderMode === kind;
        return `<button class="miracle-home-button ${active ? "miracle-home-primary" : ""}" data-builder-kind="${kind}">${BUILDER_DEVICE_LABELS[kind]}</button>`;
    }).join("");
    showPopup("実験装置ビルダー", `
        <div class="miracle-user-card" style="padding:16px;border-radius:18px;margin-bottom:14px;line-height:1.8;">
            盤面に装置を置くと、玉の動き自体が変わります。ボタンを選んでから盤面をタップしてください。配置はブラウザに保存されます。
        </div>
        <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:16px;">${rows}<button class="miracle-home-button" data-builder-kind="erase">撤去</button><button class="miracle-home-button" id="builder-mode-off">配置終了</button><button class="miracle-home-button" id="builder-clear-all">全撤去</button></div>
        <div class="miracle-user-card" style="padding:16px;border-radius:18px;line-height:1.8;">
            <b>現在の装置数:</b> ${builderDevices.length} / 36<br>
            反射板: はね返りを作る / 重力井戸: 近くの玉を引き寄せる / ワープ門: 2個で入口出口になる / 上昇ファン: 玉を上に押し返す / 奇跡増幅器: 低確率で通常玉を変質させる
        </div>
    `);
    document.querySelectorAll<HTMLElement>("[data-builder-kind]").forEach((button) => {
        button.onclick = () => {
            const kind = button.dataset.builderKind as BuilderMode;
            builderMode = kind;
            if (kind !== "warp") warpEntranceId = null;
            showSoftToast(kind === "erase" ? "撤去したい装置を盤面でタップしてください" : `${kind ? BUILDER_DEVICE_LABELS[kind] : "装置"}を置きたい場所を盤面でタップしてください`);
            closeHelpPopup();
        };
    });
    document.getElementById("builder-mode-off")!.onclick = () => { builderMode = null; warpEntranceId = null; closeHelpPopup(); showSoftToast("装置配置を終了しました"); };
    document.getElementById("builder-clear-all")!.onclick = () => {
        builderDevices = [];
        builderMode = null;
        warpEntranceId = null;
        saveBuilderDevices();
        rebuildBuilderDevicesOnBoard();
        closeHelpPopup();
        showSoftToast("すべての装置を撤去しました");
    };
}

function getDjAudioContext(): AudioContext | null {
    const AudioContextCtor = window.AudioContext || (window as any).webkitAudioContext;
    if (!AudioContextCtor) return null;
    if (!djAudioContext) djAudioContext = new AudioContextCtor();
    if (djAudioContext.state === "suspended") void djAudioContext.resume();
    return djAudioContext;
}

function playMiracleDjPulse(freq: number, duration: number, gainValue: number, type: OscillatorType = "sine"): void {
    if (!miracleDj.enabled) return;
    const ctx = getDjAudioContext();
    if (!ctx) return;
    ...
    gain.gain.exponentialRampToValueAtTime(Math.max(0.0001, gainValue * miracleDj.volume), ctx.currentTime + 0.015);
...
function playMiracleDjForRank(rank: string): void {
    if (!miracleDj.enabled) return;
    const now = Date.now();
    if (now - miracleDj.lastBeatAt < 90) return;
    miracleDj.lastBeatAt = now;
    miracleDj.combo = Math.min(16, miracleDj.combo + 1);
    const upper = rank.toUpperCase();
    const base = upper.includes("GOD") ? 196 : upper.includes("EX") ? 174 : upper.includes("UR") ? 164 : upper.includes("SSR") ? 146 : upper.includes("SR") ? 130 : 110;
    const wave: OscillatorType = miracleDj.style === "cosmic" ? "triangle" : miracleDj.style === "dance" ? "square" : "sine";
    playMiracleDjPulse(base, 0.16, 0.10, wave);
    window.setTimeout(() => playMiracleDjPulse(base * 1.5, 0.12, 0.07, wave), 90);
    if (upper.includes("EX") || upper.includes("GOD")) window.setTimeout(() => playMiracleDjPulse(base * 2, 0.28, 0.12, "sawtooth"), 180);
    if (upper.includes("GOD")) {
        [0, 120, 240, 360].forEach((delay, i) => window.setTimeout(() => playMiracleDjPulse(base * (1 + i * 0.25), 0.18, 0.11, "triangle"), delay));
    }
    saveMiracleDjState();
}

function showMiracleDjPopup(): void {
    showPopup("奇跡DJ", `
        <div class="miracle-user-card" style="padding:16px;border-radius:18px;line-height:1.8;margin-bottom:14px;">
            レアが出るほど音が重なります。実験がただの抽選ではなく、結果で変わるライブになります。
        </div>
        <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:14px;">
            <button class="miracle-home-button" id="dj-toggle">${miracleDj.enabled ? "DJ: ON" : "DJ: OFF"}</button>
            <button class="miracle-home-button" data-dj-style="soft">やさしい</button>
            <button class="miracle-home-button" data-dj-style="dance">ビート強め</button>
            <button class="miracle-home-button" data-dj-style="cosmic">宇宙</button>
            <button class="miracle-home-button" id="dj-test">試聴</button>
        </div>
        <div class="miracle-user-card" style="padding:16px;border-radius:18px;line-height:1.8;">
            現在: ${miracleDj.enabled ? "ON" : "OFF"} / スタイル: ${miracleDj.style} / コンボ音: ${miracleDj.combo}
        </div>
    `);
    document.getElementById("dj-toggle")!.onclick = () => { miracleDj.enabled = !miracleDj.enabled; saveMiracleDjState(); closeHelpPopup(); showMiracleDjPopup(); };
    document.querySelectorAll<HTMLElement>("[data-dj-style]").forEach((button) => {
        button.onclick = () => { miracleDj.style = button.dataset.djStyle as MiracleDjState["style"]; saveMiracleDjState(); playMiracleDjForRank("SSR"); closeHelpPopup(); showMiracleDjPopup(); };
    });
    document.getElementById("dj-test")!.onclick = () => playMiracleDjForRank("GOD");
}

function getPetExpNeed(): number {
    return 100 + miraclePet.level * 45;
}

function updateMiraclePetForm(): void {
    if (miraclePet.level >= 30) { miraclePet.form = "神域使い魔"; miraclePet.species = "確率ドラゴン"; }
    else if (miraclePet.level >= 18) { miraclePet.form = "星雲体"; miraclePet.species = "星読みクラゲ"; }
    else if (miraclePet.level >= 10) { miraclePet.form = "羽化体"; miraclePet.species = "ミラクルフェニックス"; }
    else if (miraclePet.level >= 5) { miraclePet.form = "研究助手体"; miraclePet.species = "確率キツネ"; }
    else { miraclePet.form = "ぷるぷる体"; miraclePet.species = "確率スライム"; }
}

function addMiraclePetExp(amount: number, reason: string): void {
    miraclePet.exp += Math.max(0, Math.floor(amount));
    miraclePet.mood = clamp(miraclePet.mood + 4, 0, 100);
    let leveled = false;
    while (miraclePet.exp >= getPetExpNeed()) {
        miraclePet.exp -= getPetExpNeed();
        miraclePet.level++;
        leveled = true;
    }
    updateMiraclePetForm();
    saveMiraclePetState();
    updatePetPanel();
    if (leveled) {
        showSoftToast(`${miraclePet.name}がLv.${miraclePet.level}になりました`);
        playMiracleDjForRank("UR");
    } else if (reason) {
        maybeShowCommentary(`使い魔「${reason}」`);
    }
}

function feedMiraclePetByRank(rank: string, label: string): void {
    const upper = rank.toUpperCase();
    const amount = upper.includes("GOD") ? 900 : upper.includes("EX") ? 420 : upper.includes("UR") ? 240 : upper.includes("SSR") ? 140 : upper.includes("SR") ? 80 : 24;
    miraclePet.favoriteRank = rank;
    miraclePet.element = upper.includes("GOD") ? "神域" : upper.includes("EX") ? "異常" : upper.includes("UR") ? "星" : upper.includes("SSR") ? "光" : "研究";
    miraclePet.totalFeed++;
    miraclePet.lastFedAt = Date.now();
    addMiraclePetExp(amount, `${label}を食べました`);
}

function gainMiraclePetExpFromRun(): void {
    const rareCount = Object.values(specialCreated).reduce((a, b) => a + b, 0);
    const amount = Math.min(600, Math.floor(finishedCount / 80) + rareCount * 25 + Math.floor(runScore / 20000));
    if (amount > 0) addMiraclePetExp(amount, "実験ログを食べました");
}

function ensurePetPanel(): void {
    if (petPanel) return;
    petPanel = document.createElement("div");
    petPanel.style.position = "fixed";
    petPanel.style.left = isMobile ? "10px" : "18px";
    petPanel.style.bottom = isMobile ? "78px" : "18px";
    petPanel.style.zIndex = "88";
    petPanel.style.maxWidth = isMobile ? "calc(100vw - 20px)" : "320px";
    petPanel.style.padding = "10px 14px";
    petPanel.style.borderRadius = "18px";
    petPanel.style.background = "rgba(15,23,42,.72)";
    petPanel.style.color = "#fff";
    petPanel.style.boxShadow = "0 16px 40px rgba(0,0,0,.28)";
    petPanel.style.border = "1px solid rgba(255,255,255,.18)";
    petPanel.style.fontWeight = "900";
    petPanel.style.cursor = "pointer";
    petPanel.onclick = () => showMiraclePetPopup();
    document.body.appendChild(petPanel);
}

function updatePetPanel(): void {
    ensurePetPanel();
    const need = getPetExpNeed();
    const p = clamp(miraclePet.exp / need, 0, 1);
    petPanel.innerHTML = `
        <div style="display:flex;gap:10px;align-items:center;">
            <div style="font-size:28px;">${miraclePet.level >= 30 ? "🐉" : miraclePet.level >= 18 ? "🪼" : miraclePet.level >= 10 ? "🔥" : miraclePet.level >= 5 ? "🦊" : "🫧"}</div>
            <div style="flex:1;min-width:0;">
                <div style="font-size:14px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${escapeHtml(miraclePet.name)} Lv.${miraclePet.level} / ${escapeHtml(miraclePet.species)}</div>
                <div style="height:7px;background:rgba(255,255,255,.22);border-radius:999px;overflow:hidden;margin-top:5px;"><div style="height:100%;width:${(p * 100).toFixed(1)}%;background:#facc15;"></div></div>
            </div>
        </div>`;
}

function showMiraclePetPopup(): void {
    const need = getPetExpNeed();
    showPopup("奇跡を育てる使い魔", `
        <div class="miracle-user-card" style="padding:18px;border-radius:20px;margin-bottom:14px;line-height:1.8;">
            <div style="font-size:42px;margin-bottom:8px;">${miraclePet.level >= 30 ? "🐉" : miraclePet.level >= 18 ? "🪼" : miraclePet.level >= 10 ? "🔥" : miraclePet.level >= 5 ? "🦊" : "🫧"}</div>
            <b>${escapeHtml(miraclePet.name)}</b> / Lv.${miraclePet.level} / ${escapeHtml(miraclePet.species)}<br>
            形態: ${escapeHtml(miraclePet.form)} / 属性: ${escapeHtml(miraclePet.element)} / 機嫌: ${miraclePet.mood}<br>
            EXP: ${miraclePet.exp} / ${need} / 食べた奇跡: ${miraclePet.totalFeed}
        </div>
        <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:14px;">
            <button class="miracle-home-button" id="pet-feed-test">実験ログをあげる</button>
            <button class="miracle-home-button" id="pet-rename">名前変更</button>
            <button class="miracle-home-button" id="pet-reset">使い魔リセット</button>
        </div>
        <div class="miracle-user-card" style="padding:16px;border-radius:18px;line-height:1.8;">
            レアを引くと自動で成長します。GODやEXを食べると大きく育ち、姿も変わります。
        </div>
    `);
    document.getElementById("pet-feed-test")!.onclick = () => { addMiraclePetExp(40, "研究メモを食べました"); closeHelpPopup(); showMiraclePetPopup(); };
    document.getElementById("pet-rename")!.onclick = () => {
        const name = window.prompt("使い魔の名前", miraclePet.name);
        if (name && name.trim()) { miraclePet.name = name.trim().slice(0, 16); saveMiraclePetState(); updatePetPanel(); closeHelpPopup(); showMiraclePetPopup(); }
    };
    document.getElementById("pet-reset")!.onclick = () => {
        if (!window.confirm("使い魔を初期化しますか？")) return;
        miraclePet = createDefaultPetState();
        saveMiraclePetState();
        updatePetPanel();
        closeHelpPopup();
        showMiraclePetPopup();
...
function handlePachinkoYakumonoPassage(yakumono: Matter.Body, drop: Matter.Body): void {
    ...
    playMiracleDjForRank(special.rank);
    feedMiraclePetByRank(special.rank, special.label);
...
Events.on(engine, "afterUpdate", () => {
    ...
    for (const body of engine.world.bodies) {
        ...
        applyBuilderDeviceEffects(body);
