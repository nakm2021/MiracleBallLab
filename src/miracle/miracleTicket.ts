...
export function normalizeMiracleTicketState(value: unknown): MiracleTicketState {
    ...
    return {
        ...
        history: Array.isArray(raw.history)
            ? raw.history.slice(0, 50).map((entry): MiracleTicketHistoryEntry => {
                  const x = entry && typeof entry === "object" ? (entry as Partial<MiracleTicketHistoryEntry>) : {};
                  const kind: TicketKind = x.kind === "rare" || x.kind === "divine" ? x.kind : "normal";
                  return {
...
export function loadMiracleTicketState(): MiracleTicketState {
    try {
        return normalizeMiracleTicketState(JSON.parse(localStorage.getItem(STORAGE_KEY) || "null"));
    } catch {
        return createInitialMiracleTicketState();
    }
}

export function saveMiracleTicketState(state: MiracleTicketState): void {
    try {
        localStorage.setItem(STORAGE_KEY, JSON.stringify(normalizeMiracleTicketState(state)));
    } catch {}
}

function addHistory(
    state: MiracleTicketState,
    kind: TicketKind,
    amount: number,
    label: string,
    reason: string,
...
export function awardTicketsForRank(
    state: MiracleTicketState,
    rank: string,
    label: string,
): { state: MiracleTicketState; reward: MiracleTicketReward } {
...
export function spendMiracleTickets(
    state: MiracleTicketState,
    cost: Partial<Record<TicketKind, number>>,
): { ok: boolean; state: MiracleTicketState; message: string } {
    const next = normalizeMiracleTicketState(state);
    ...
    if (next.normal < normal || next.rare < rare || next.divine < divine)
        return { ok: false, state: next, message: "チケットが足りません" };
    next.normal -= normal;
    next.rare -= rare;
    next.divine -= divine;
