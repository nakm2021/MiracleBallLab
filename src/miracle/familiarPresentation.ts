...
export function getMiracleTicketHtml(state: MiracleTicketState): string {
    ...
    `,
            ...
            .join("") ||
        `<p style="opacity:.75;">まだチケット履歴はありません。SR以上の奇跡を観測すると集まりやすいです。</p>`;
    return `
...
export function getFamiliarExpeditionHtml(
    ...
    const historyRows =
        state.history
            ...
            .map(
                (x) =>
                    `<div style="padding:8px 0;border-bottom:1px solid rgba(80,90,120,.12);"><b>${escapeHtml(x.title)}</b> / ${new Date(x.at).toLocaleString()}</div>`,
            )
            .join("") || `<p style="opacity:.7;">遠征履歴はまだありません。</p>`;
    return `
