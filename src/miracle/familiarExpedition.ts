...
export const FAMILIAR_EXPEDITION_PLANS: FamiliarExpeditionPlan[] = [
    ...
    },
    {
        ...
        title: "3時間・深夜研究所",
        minutes: 180,
        description: "長めの遠征。秘密ノートやレアチケットを狙えます。",
        xp: 520,
        affection: 80,
        ticketNormal: 6,
        ticketRare: 1,
...
export function normalizeFamiliarExpeditionState(value: unknown): FamiliarExpeditionState {
    ...
    return {
        active:
            active && typeof active.planId === "string"
                ? {
...
export function loadFamiliarExpeditionState(): FamiliarExpeditionState {
    try {
        return normalizeFamiliarExpeditionState(JSON.parse(localStorage.getItem(STORAGE_KEY) || "null"));
    } catch {
        return createInitialFamiliarExpeditionState();
    }
}

export function saveFamiliarExpeditionState(state: FamiliarExpeditionState): void {
    try {
        localStorage.setItem(STORAGE_KEY, JSON.stringify(normalizeFamiliarExpeditionState(state)));
    } catch {}
}
...
export function startFamiliarExpedition(
    ...
    familiarKind: FamiliarKind,
    now = Date.now(),
): { ok: boolean; state: FamiliarExpeditionState; message: string } {
...
export function getFamiliarExpeditionProgress(
    state: FamiliarExpeditionState,
    now = Date.now(),
): { active: boolean; complete: boolean; percent: number; remainingMs: number; plan?: FamiliarExpeditionPlan } {
...
export function claimFamiliarExpedition(
    state: FamiliarExpeditionState,
    now = Date.now(),
): { ok: boolean; state: FamiliarExpeditionState; reward?: FamiliarExpeditionHistoryEntry; message: string } {
    const next = normalizeFamiliarExpeditionState(state);
    ...
    if (!plan) {
        ...
        saveFamiliarExpeditionState(next);
        return { ok: false, state: next, message: "遠征データを整理しました" };
    }
    if (now < active.endsAt) return { ok: false, state: next, message: "まだ遠征中です" };
    const reward: FamiliarExpeditionHistoryEntry = {
        id: createId(),
        at: now,
        planId: plan.id,
        title: plan.title,
        familiarKind: active.familiarKind,
        xp: plan.xp,
        affection: plan.affection,
        ticketNormal: plan.ticketNormal,
        ticketRare: plan.ticketRare,
        noteId: plan.noteId,
    };
    next.active = undefined;
