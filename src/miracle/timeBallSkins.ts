...
export function chooseTimeBallSkin(enabled: boolean, random: () => number, now = new Date()): TimeBallSkin {
    ...
    if (theme === "morning") return roll < 0.2 ? "drop" : "normal";
    if (theme === "day") return roll < 0.05 ? "gloss" : "normal";
    if (theme === "evening") return roll < 0.2 ? "spark" : "normal";
    if (theme === "night") {
        if (roll < 0.15) return "star";
        if (roll < 0.2) return "moon";
        return "normal";
    }
    if (roll < 0.2) return "darkShard";
...
export function drawTimeBallSkinIcon(
    ...
    if (skin === "drop") {
        ...
        context.beginPath();
        context.ellipse(-radius * 0.22, -radius * 0.26, radius * 0.16, radius * 0.28, -0.45, 0, Math.PI * 2);
        context.fill();
    } else if (skin === "spark") {
        ...
        context.beginPath();
        context.arc(0, 0, radius * 0.98, Math.PI * 0.22, Math.PI * 1.78);
        ...
        if (skin === "swordShard") {
            ...
            context.beginPath();
            context.moveTo(-radius * 0.15, -radius * 0.72);
            context.lineTo(radius * 0.18, radius * 0.72);
            context.stroke();
        }
    } else if (skin === "coin") {
        context.beginPath();
        context.ellipse(0, 0, radius * 0.92, radius * 0.76, 0, 0, Math.PI * 2);
        context.fill();
        context.stroke();
        context.strokeStyle = "rgba(95,58,0,.65)";
        context.lineWidth = Math.max(1, radius * 0.08);
        context.beginPath();
        context.ellipse(0, 0, radius * 0.58, radius * 0.46, 0, 0, Math.PI * 2);
        context.stroke();
    } else if (skin === "heart") {
        context.beginPath();
        ...
        context.beginPath();
        context.arc(-radius * 0.28, -radius * 0.32, radius * 0.24, 0, Math.PI * 2);
        context.fill();
    }
    context.restore();
}
